...
public class CaptureContextRepository {
    ...
    public void setCurrent(String documentationType, String activityProjectId) {
        ...
        cv.put("shot_type", CameraPrefs.SHOT_GENERAL);
    ...
    private void ensureRow(SQLiteDatabase db) {
        db.execSQL("INSERT OR IGNORE INTO " + GeoDbHelper.TABLE_CAPTURE_CONTEXT +
                "(context_id, monitoring_type, activity_project_id, shot_type, pending_monitoring_type, pending_activity_project_id, pending_shot_type, updated_at) " +
                "VALUES (1, 'UNSPECIFIED', NULL, 'GENERAL', 'UNSPECIFIED', NULL, 'GENERAL', datetime('now'))");
    }

    private String normalizeType(String value) {
        if (value != null && CameraPrefs.DOC_INFRA.equalsIgnoreCase(value.trim())) return CameraPrefs.DOC_INFRA;
        if (value != null && CameraPrefs.DOC_PROJECT.equalsIgnoreCase(value.trim())) return CameraPrefs.DOC_PROJECT;
        if (value != null && CameraPrefs.DOC_ACTIVITY.equalsIgnoreCase(value.trim())) return CameraPrefs.DOC_ACTIVITY;
        if (value != null && CameraPrefs.DOC_PROJECT_ACTIVITY.equalsIgnoreCase(value.trim())) return CameraPrefs.DOC_PROJECT_ACTIVITY;
        if (value != null && CameraPrefs.DOC_PERSONAL.equalsIgnoreCase(value.trim())) return CameraPrefs.DOC_PERSONAL;
        return "UNSPECIFIED";
    }

    private String normalizeActivityProjectId(String type, String value) {
        // Only the legacy PROJECT_ACTIVITY route carries activity_project_id.
        // Standalone PROJECT and ACTIVITY use their unified tbl_project UUID as siteId.
