...
public class SiteDatesActivity extends AppCompatActivity {
    ...
    private void configureToolbarIdentity() {
        ...
        if (TYPE_PERSONAL.equals(captureType)) {
            ...
            toolbar.setSubtitle(divisionCode.isEmpty() ? "Activity" : "Activity • " + divisionCode);
        } else if (TYPE_PROJECT_ACTIVITY.equals(captureType)) {
            toolbar.setSubtitle(divisionCode.isEmpty() ? "Project Activity" : "Project Activity • " + divisionCode);
        } else {
    ...
    private void openGeoCam() {
        ...
        Intent cam = new Intent(this, ph.gov.geocamera.presentation.geocamera.GeoCameraActivity.class);
    ...
    private void hideDrop(View v) {
        if (v == null) return;
        v.animate().cancel();
        v.animate().alpha(0f).translationY(-dp(8)).scaleX(.985f).scaleY(.985f)
                .setDuration(140)
    ...
    private void pressEffect(View v) {
        if (v == null) return;
        v.animate().scaleX(.985f).scaleY(.985f).alpha(.92f).setDuration(70)
                .withEndAction(() -> v.animate().scaleX(1f).scaleY(1f).alpha(1f).setDuration(100).start())
                .start();
    }
    ...
    private void showRemarksDialog(String sessionDate) {
        ...
        com.google.android.material.textfield.TextInputLayout tilRemarks = view.findViewById(R.id.tilRemarks);
        com.google.android.material.textfield.TextInputEditText etRemarks = view.findViewById(R.id.etRemarks);
    ...
    private void loadYearSpinner() {
        ...
        ArrayAdapter<String> ad = new ArrayAdapter<>(this, android.R.layout.simple_spinner_dropdown_item, years);
    ...
    private void loadMonthSpinner() {
        ...
        for (int n = 1; n <= 12; n++) {
            months.add(new MonthOption(String.format(Locale.US, "%02d", n), names[n - 1]));
        }

        ArrayAdapter<MonthOption> ad = new ArrayAdapter<>(this, android.R.layout.simple_spinner_dropdown_item, months);
        spMonth.setAdapter(ad);
        int sel = 0;
        for (int idx = 0; idx < months.size(); idx++) {
            if (months.get(idx).value.equals(monthOrAll)) {
                sel = idx;
                break;
            }
        }
    ...
    /**
     * Uses the repository's existing date query contract.
     * getDatesForSite columns:
     * 0 sessiondate, 1 groupid, 2 remarks, 3 total, 4 uploaded,
     * 5 uploading, 6 failed, 7 pending, 8 unsynced, 9 filename, 10 timestamp.
     */
    private void loadDates() {
        List<SiteDatesAdapter.DateItem> list = new ArrayList<>();
        Cursor c = null;
        try {
            c = imageRepo.getDatesForSite(siteId, "ALL", yearOrAll, monthOrAll);
            while (c != null && c.moveToNext()) {
                SiteDatesAdapter.DateItem d = new SiteDatesAdapter.DateItem();
                d.sessionDate = c.getString(0);
                d.groupId = c.getString(1);
                d.remarks = c.getString(2);
                d.totalPhotos = c.getInt(3);
                d.uploadedPhotos = c.getInt(4);
                d.uploadingPhotos = c.getInt(5);
                d.failedPhotos = c.getInt(6);
                d.pendingPhotos = c.getInt(7);
                d.unsyncedPhotos = c.getInt(8);
                d.latestFilename = c.getString(9);
                d.latestTimestamp = c.getString(10);
                list.add(d);
            }
        } finally {
            if (c != null) c.close();
        ...
        adapter.submit(list);
        View empty = findViewById(R.id.datesEmptyState);
