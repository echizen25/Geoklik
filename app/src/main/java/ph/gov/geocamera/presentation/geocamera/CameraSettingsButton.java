...
/**
 * Simple camera settings menu using the original list-style dialog.
 * Keeps Change Project/Site, GPS mode and flash in one compact window.
 */
public class CameraSettingsButton extends AppCompatImageButton {
    ...
    public void setOnClickListener(@Nullable OnClickListener l) {
        // Retain the Activity listener for compatibility/fallback only.
        activityListener = l;
    }
                ...
                .setItems(items, (dialog, which) -> {
                    ...
                    if (which == 1) {
                        ...
                        Toast.makeText(
                                ...
                                enableIndoor
                                        ? "GPS Mode: Indoor Assist"
                                        : "GPS Mode: Outdoor (GPS Only)",
                                Toast.LENGTH_SHORT
                        ).show();

                        // GeoCamera caches this flag at startup; refresh once so
                        // the new GPS mode takes effect immediately.
                        if (hostActivity != null && !hostActivity.isFinishing()) {
                            hostActivity.recreate();
                        }
                        return;
