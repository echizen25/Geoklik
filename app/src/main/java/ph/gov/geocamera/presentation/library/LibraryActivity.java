...
public class LibraryActivity extends BaseTopAppBarActivity {
    ...
    private MaterialCardView cardEmpty, cardSearch;
    ...
    @Override protected int getLayoutResId() { return R.layout.activity_library; }
    @Override protected String getScreenTitle() { return "Projects Library"; }
    ...
    private boolean onToolbarMenuClick(MenuItem item) {
        ...
        if (id == R.id.action_sync) {
            ...
            syncProjectsFromApiIfOnline(true); return true;
        }
        if (id == R.id.action_geocam) { startActivity(new Intent(this, GeoCameraActivity.class)); return true; }
    ...
    private void setupPullRefresh() {
        ...
        swipeRefresh.setOnRefreshListener(() -> {
            ...
            if (!isInternetAvailable()) {
                ...
                swipeRefresh.setRefreshing(false); return;
            }
    ...
    private void setupSearch() {
        ...
        etSearch.addTextChangedListener(new android.text.TextWatcher() {
            ...
            @Override public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (adapter != null) { adapter.getFilter().filter(s); etSearch.post(LibraryActivity.this::updateFilteredState); }
            }
    ...
    private void toggleSearch() {
        ...
        if (cardSearch != null) cardSearch.setVisibility(searchVisible ? View.VISIBLE : View.GONE);
        if (!searchVisible && etSearch != null) { etSearch.setText(""); if (adapter != null) adapter.getFilter().filter(""); updateFilteredState(); }
        if (searchVisible && etSearch != null) etSearch.requestFocus();
    }
    ...
    private void loadProjectsFromLocal() {
        try {
            ...
            items.clear();
            if (data != null) {
                for (ProjectListItem item : data) {
                    if (item == null) continue;
                    item.projectType = projectRepository.getProjectTypeById(item.projectId);
                    item.divisionCode = projectRepository.getDivisionCodeByProjectId(item.projectId);
                    items.add(item);
                }
            }
            ...
            updateFilteredState();
        } catch (Exception e) { Log.e(TAG, "loadProjectsFromLocal() error", e); }
    }

    private void updateFilteredState() {
        int count = adapter == null ? 0 : adapter.getFilteredCount();
        boolean empty = count == 0;
        if (cardEmpty != null) cardEmpty.setVisibility(empty ? View.VISIBLE : View.GONE);
        if (rvProjects != null) rvProjects.setVisibility(empty ? View.GONE : View.VISIBLE);
    }

    private void syncProjectsFromApiIfOnline(boolean showToast) {
        if (!isInternetAvailable()) {
            if (showToast) Toast.makeText(this, "No internet connection", Toast.LENGTH_SHORT).show();
        ...
        executor.execute(() -> {
            try {
                ...
                runOnUiThread(() -> {
                    loadProjectsFromLocal();
                    if (swipeRefresh != null) swipeRefresh.setRefreshing(false);
                    if (showToast) Toast.makeText(this, "Projects synced", Toast.LENGTH_SHORT).show();
                });
            } catch (Exception e) {
                Log.e(TAG, "Sync failed", e);
                runOnUiThread(() -> {
                    if (swipeRefresh != null) swipeRefresh.setRefreshing(false);
                    if (showToast) Toast.makeText(this, "Sync failed: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                });
            }
        });
    }
